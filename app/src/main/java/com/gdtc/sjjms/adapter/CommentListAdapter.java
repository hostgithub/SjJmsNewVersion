...
public class CommentListAdapter extends RecyclerView.Adapter<BaseViewHolder> {
    ...
    private ArrayList<CommentList.ResultsBean> datas;
    private Context mContext;
    private int viewFooter;
    private View footerView;
    private OnItemClickListener mOnItemClickListener;

    public CommentListAdapter(Context context, ArrayList<CommentList.ResultsBean> datas) {
        this.datas = datas;
        this.mContext = context;
    }

    public void replaceAll(List<CommentList.ResultsBean> elements) {
        if (datas.size() > 0) {
            datas.clear();
        }
    ...
    public void addAll(List<CommentList.ResultsBean> elements) {
        datas.addAll(elements);
        notifyDataSetChanged();
    }
    @Override
    public BaseViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == ITEM_NOIMAGE) {
            return new BaseViewHolder(LayoutInflater.from(mContext).inflate(R.layout.item_comment, parent, false));
...
//            holder.itemView.set
            ...
            final CommentList.ResultsBean item = datas.get(position);
...
//            }
            ...
            ImageLoaderUtils.loadingImg(mContext, (ImageView) holder.getView(R.id.user_image), item.getImage());
            holder.setText(R.id.user_name, item.getName());
            holder.setText(R.id.comment,  item.getEvaluate());
