...
public class HomeTuijianAdapter extends RecyclerView.Adapter<BaseViewHolder> {
    private static final int TYPE_FOOTER = 0;
    private static final int ITEM_NOIMAGE = 1;
    private static final int ITEM_HASIMAGE = 2;

    private ArrayList<HomeTuijian.ResultsBean> datas;
    private Context mContext;
    private int viewFooter;
    private View footerView;
    private OnItemClickListener mOnItemClickListener;

    public HomeTuijianAdapter(Context context, ArrayList<HomeTuijian.ResultsBean> datas) {
        this.datas = datas;
        this.mContext = context;
    }

    public void replaceAll(List<HomeTuijian.ResultsBean> elements) {
        if (datas.size() > 0) {
            datas.clear();
        }
    ...
    public void addAll(List<HomeTuijian.ResultsBean> elements) {
        datas.addAll(elements);
        notifyDataSetChanged();
    }
    @Override
...
//            holder.itemView.set
            ...
            final HomeTuijian.ResultsBean item = datas.get(position);


            ImageLoaderUtils.loadingImg(mContext, (ImageView) holder.getView(R.id.cover), item.getGoodsImage());
...
//            }
            ...
            holder.setText(R.id.tv_price, "￥"+item.getConsumption()+"/人");
            holder.setText(R.id.title, item.getGoodsName());
            holder.setText(R.id.tv_location, item.getRegional());
            holder.setText(R.id.content, item.getCategory().substring(0,item.getCategory().length()-1));
...
//            });
    ...
    public int getItemViewType(int position) {
        ...
        if (TextUtils.isEmpty(datas.get(position).getGoodsImage())) {
            type = ITEM_NOIMAGE;
        } else {
            type = ITEM_HASIMAGE;
        }
